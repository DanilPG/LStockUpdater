...
def list_stores_from_assets() -> Dict[str, str]:
	# Нормализуем путь к assets/yandex независимо от cwd
	project_root = _get_project_root()
	base = os.path.join(project_root, "assets", "yandex")
	if not os.path.isdir(base):
		# fallback: относительный путь от cwd
		base = os.path.join(os.getcwd(), "assets", "yandex")
	result: Dict[str, str] = {}
...
def update_stocks(selected_stores: Optional[List[str]] = None) -> str:
	logger = get_logger()
	config = load_config()
	stores_meta: Dict[str, Dict] = config.setdefault("stores", {}).setdefault("yandex", {})
	...
	if not stores_to_update:
		return "Нет выбранных магазинов Яндекс.Маркет"

	results: List[str] = []
	for store in stores_to_update:
		logger.info("Обновление остатков Яндекс.Маркет (скрипт): %s", store)
...
def reset_stocks(selected_stores: Optional[List[str]] = None) -> str:
	...
	# 1) assets/yandex/<store>_reset.py
	# 2) assets/yandex/reset/<store>.py
	project_root = _get_project_root()
	assets_dir = os.path.join(project_root, "assets", "yandex")
	results: List[str] = []
	for store in selected_stores:
		...
		if not script_path:
			...
			continue
		logger.info("Обнуление остатков Яндекс.Маркет (скрипт): %s", store)
