...
def list_stores_from_assets() -> Dict[str, str]:
	# Нормализуем путь к assets/wb независимо от cwd
	project_root = _get_project_root()
	base = os.path.join(project_root, "assets", "wb")
	if not os.path.isdir(base):
		# fallback: относительный путь от cwd
		base = os.path.join(os.getcwd(), "assets", "wb")
	result: Dict[str, str] = {}
...
def update_stocks(selected_stores: Optional[List[str]] = None) -> str:
	logger = get_logger()
	config = load_config()
	stores_meta: Dict[str, Dict] = config.setdefault("stores", {}).setdefault("wb", {})
	...
	if not stores_to_update:
		return "Нет выбранных магазинов Wildberries"

	results: List[str] = []
	for store in stores_to_update:
		logger.info("Обновление остатков Wildberries (скрипт): %s", store)
...
def reset_stocks(selected_stores: Optional[List[str]] = None) -> str:
	...
	# 1) assets/wb/<store>_reset.py
	# 2) assets/wb/reset/<store>.py
	project_root = _get_project_root()
	assets_dir = os.path.join(project_root, "assets", "wb")
	results: List[str] = []
	for store in selected_stores:
		...
		if not script_path:
			...
			continue
		logger.info("Обнуление остатков Wildberries (скрипт): %s", store)
